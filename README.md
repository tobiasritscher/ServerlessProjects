...
- [Overview](/Labs/Lab02/overview/) / [function](https://europe-west6-daring-runway-326914.cloudfunctions.net/display-state-info) 
  ...
  We first intended to chose Amazon Web Services as the provider for the databse but then we realised that it wasn't possible to get a free database with them. In the end we settled for a Firebase (Firestore) database since that was free and easy to use. Since both GCF and Firbase is from Google we decided to make the analytics page in Java so that we have a polyglot application.  
