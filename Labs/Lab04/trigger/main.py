...
class Config:
    BUCKET_NAME         = "gcf-sources-454851464893-europe-west6-data"
...
def handle_post(request, form):
    if form.validate_on_submit():
        ...
        # create unique name for data
        uid = uuid.uuid4() 
        filename = f"user-information-{uid.hex}.txt"
        blob = bucket.blob(filename)
        blob.upload_from_string(json.dumps(res.to_obj()))

        return flask.render_template("form_submitted.html", title="Sucess submitting", result=res, filename=filename)
    else:
        return handle_invalid_form(request,form)

def handle_get(request, form):
    # make the linter shutup
    _ = request
